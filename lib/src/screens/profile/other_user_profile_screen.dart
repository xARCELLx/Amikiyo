...
class _OtherUserProfileScreenState extends State<OtherUserProfileScreen> {
    ...
    try {
      ...
      if (profileResponse.statusCode == 200) {
        ...
        final postsResponse = await http.get(
          Uri.parse('${ApiConstants.baseUrl}/posts/user/${widget.userId}/'),
          headers: {'Authorization': 'Token $token'},
        );
  ...
  Widget build(BuildContext context) {
    ...
    final profileImage = profileData!['profile_image'] ?? Constants.defaultProfilePath;
            ...
            if (board.isNotEmpty)
              Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Anime Board', style: TextStyle(color: Color(0xFF00FF7F), fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    // You can expand this later — for now show top 3 + watched count
                    Text('Top 3: ${(board['top_three'] as List?)?.length ?? 0}', style: const TextStyle(color: Colors.white)),
                    Text('Watched: ${(board['watched'] as List?)?.length ?? 0}', style: const TextStyle(color: Colors.white)),
                  ],
                ),
              ),
